...
for file_number = iniFile:finalFile
    ...
    i_crop3 = imadjust(i_crop3,[0.3 0.6]);
    figure(2), subplot(1,3,1),imshow(i_crop3);
...
%     J = roifilt2(H, roi, i_crop2);
 cop = i_crop2;
 cop(~roi) = 255;
 cop_min=imextendedmin(cop, 40, 4);
%  darkpixel = min(cop(:));
%  [r,c] = find(cop == darkpixel);
%  D = bwdist(i_crop3);
%  cop2 = watershed(D);
% D = imhist(cop);
% cop(r,c) = 0;
% se = strel('disk',2);
% se2 = strel('diamond',5);
% cop_dilate = imdilate(cop_min, se);
% cop_erode = imerode(cop_dilate, se2);

% cop_erode2 = imerode(cop_dilate, se);
% cop_dilate2 = imdilate(cop_erode2, se);
 figure(2), 
 subplot(1,3,2), imshow(cop_min);

 
%  subplot(1,3,3), imshow(cop_erode);
