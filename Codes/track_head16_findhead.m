n%% Start with nothing (an empty workspace)
clear all;
clc;
% close all;
        ...
        if (num_object == 1 )
            if head_stats(1).MeanIntensity < 100
                if head_number > 0
                    ...
                    in = inpolygon(x_cent, y_cent, xv, yv);
                    if in
                        head1 = 1;
                        head_found = 1;
                    end
            ...
            head1 = find([head_stats.MeanIntensity] < 100);
            tail1 = find([head_stats.MeanIntensity] > 100 & [head_stats.Area] == area_head(2));
            if head1, head_found = 1; else head1 = find([head_stats.Area] == area_head(1)); end
            if tail1, tail_found = 1; else tail1 = find([head_stats.Area] == area_head(2)); end
        end
end
if head_number > 0 & head_found == 1
        ...
        %         if dist_Headbox(file_number) > 5
        %             dist_Headbox(file_number) = dist_Headbox(file_number - 1);
    ...
    if tail_number > 0 & tail_found == 1
        ...
        %             dist_Tailbox(file_number) = dist_Tailbox(file_number - 1);
