...
for file_number = iniFile:finalFile
    ...
    boxsize = 10;
    bbox_prop = regionprops(i2,'BoundingBox','Area');
    ...
    %if file_number == 1
%         figure(1),imshow(i2), hold on, rectangle('Position', nextbboxref(fileCount,:),'EdgeColor','w'), hold off;
...
%         figure(1),imshow(i2), hold on, rectangle('Position', nextbboxref(fileCount,:),'EdgeColor','w'), hold off;
    ...
    i5 = bwmorph(i2, 'skel', Inf);
    i5 = bwmorph(i5, 'bridge', 3);
    i6 = bwmorph(i5, 'spur', 5);
    E = bwmorph(i6, 'endpoints');
    [y,x] = find(E);
    figure (1), imshow(i6);
    figure (2), imshow(i2);
