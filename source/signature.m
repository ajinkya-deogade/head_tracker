function [x0, y0, coordinate_real] = signature(b, varargin)
...
coordinate_real(:,1) = x_real(:,1);
coordinate_real(:,2) = y_real(:,1);
coordinate_real(:,3) = xc(:,1);
coordinate_real(:,4) = yc(:,1);
coordinate_real(:,5) = theta(:,1);
coordinate_real(:,6) = rho(:,1);
...
[w, u, v] = unique(coordinate_real(:,5),'stable');
coordinate_real(sort(u));
...
if coordinate_real(end, 5) == coordinate_real(5) + 360
   coordinate_real = coordinate_real(1:end - 1, :);
end
